...
export const addPortfolio = (
  ...
  if (!name || portfolios.includes(name)) {
    return toast.error("Portfolio already exists or invalid name!");
  }
...
export const deletePortfolio = (
  ...
  if (name === "MainPortfolio")
    return toast.error("Cannot delete default portfolio.");
  ...
  if (window.confirm(`Delete "${name}" and all its transactions?`)) {
    ...
    setPortfolios(updatedPortfolios);
    setTransactions(updatedTransactions);
    setActivePortfolio(updatedData.selectedPortfolio);
    syncToFirestore(updatedData);
