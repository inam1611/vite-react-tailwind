...
import { doc, getDoc, setDoc, serverTimestamp } from "firebase/firestore";
import { onAuthStateChanged } from "firebase/auth";
import { usePortfolioContext } from "../../contexts/PortfolioContext"; // ✅ use global
import toast from "react-hot-toast";

let inMemoryCache = null;

export const useTransactionsData = () => {
  const { activePortfolio, setActivePortfolio } = usePortfolioContext(); // ✅ global portfolio
  ...
  useEffect(() => {
    if (inMemoryCache) {
      ...
      setPortfolios(data.portfolios);
      setTransactions(data.transactions);
      if (data.selectedPortfolio) setActivePortfolio(data.selectedPortfolio); // ✅ sync global
      setLoading(false);
      return;
    }
    ...
    if (cached) {
      ...
      setPortfolios(parsed.portfolios || ["Main Portfolio"]);
      setTransactions(parsed.transactions || []);
      if (parsed.selectedPortfolio) setActivePortfolio(parsed.selectedPortfolio); // ✅
    }
  }, [setActivePortfolio]);
    ...
    const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
      ...
      try {
        ...
        if (userDoc.exists()) {
          ...
          setPortfolios(data.portfolios || ["Main Portfolio"]);
          setTransactions(data.transactions || []);
          if (data.selectedPortfolio) setActivePortfolio(data.selectedPortfolio);
          inMemoryCache = data;
          localStorage.setItem("cachedTransactionsData", JSON.stringify(data));
        } else {
          const defaultData = {
          ...
          await setDoc(userDocRef, defaultData);
          inMemoryCache = defaultData;
          setActivePortfolio("Main Portfolio");
          localStorage.setItem("cachedTransactionsData", JSON.stringify(defaultData));
        }
      } catch (err) {
        console.error("Error loading data:", err);
  ...
  const syncToFirestore = async (updatedData) => {
    ...
    try {
      ...
      await setDoc(userDocRef, {
        ...updatedData,
        selectedPortfolio: activePortfolio, // ✅ use global
        lastUpdated: serverTimestamp(),
      });
      inMemoryCache = updatedData;
      localStorage.setItem(
        "cachedTransactionsData",
        JSON.stringify({ ...updatedData, selectedPortfolio: activePortfolio })
      );
    } catch (err) {
      console.error("Error syncing:", err);
      toast.error("Failed to sync data.");
