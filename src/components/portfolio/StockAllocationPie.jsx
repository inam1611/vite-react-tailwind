...
// const StockAllocationPie = ({ holdings }) => {
...
//   if (!pieData.length) return <p className="text-gray-500 text-center">No holdings to display</p>;
...
const StockAllocationPie = ({ holdings }) => {
  const pieData = useMemo(() => {
    const total = holdings.reduce(
      (sum, h) => sum + Number(h.totalCost || 0),
      0
    );
    return holdings.map((h) => ({
      name: h.symbol || "Unknown",
      value: Number(h.totalCost) || 0,
      percentage: total > 0 ? (Number(h.totalCost || 0) / total) * 100 : 0,
    }));
  }, [holdings]);

  if (!pieData.length)
    return <p className="text-gray-500 text-center">No holdings to display</p>;
